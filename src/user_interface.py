...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.voice_combobox.addItems(
            ["Adam", "Arnold", "Emily", "Harry", "Josh", "Patrick"]
        )
        ...
        self.router_model_box.addItems(
            [
                "claude-3-5-sonnet-20240620",
                "gpt-4o",
                "gemini-1.5-flash",
...
class ProcrastinationApp(QWidget):
    ...
    def initUI(self):
        ...
        subtitle_4 = (
            "And if you dare to procrastinate, I will make you pay the price ;)"
        )
        ...
        if task_description:
            ...
            arguments.extend(
                [
                    ...
                    self.settings["voice"],
                    "--delay_time",
                    str(self.settings["delay_time"]),
                    "--initial_delay",
                    str(self.settings["initial_delay"]),
                    "--countdown_time",
                    str(self.settings["countdown_time"]),
                    "--user_name",
    ...
    def handle_stdout(self):
        ...
        elapsed_time = (
            QTime(0, 0)
            .addSecs(self.start_time.secsTo(QTime.currentTime()))
            .toString("hh:mm:ss")
        )
        ...
        if (
            "procrastinating" in output.lower()
            and api_name_to_colloquial[self.settings["router_model"]] in output
        ):
            format.setForeground(QColor("orange"))
        elif (
            "procrastinating" in output.lower()
            and api_name_to_colloquial[self.settings["model"]] in output
        ):
    ...
    def update_timer(self):
        if self.start_time:
            elapsed_time = QTime(0, 0).secsTo(QTime.currentTime()) - QTime(0, 0).secsTo(
                self.start_time
            )
    ...
    def resizeEvent(self, event):
        background_image = QPixmap(
            os.path.dirname(self.cur_dir) + "/assets/space_2.jpg"
    ...
    def apply_settings(self):
        ...
        self.settings_dialog.initial_delay_spinbox.setValue(
            self.settings["initial_delay"]
        )
        self.settings_dialog.countdown_time_spinbox.setValue(
        ...
        self.settings_dialog.router_model_box.setCurrentText(
            self.settings["router_model"]
        )
...
if __name__ == "__main__":
    ...
    app.setWindowIcon(
        QIcon(os.path.dirname(os.path.dirname(__file__)) + "/assets/icon_rounded.png")
    )
